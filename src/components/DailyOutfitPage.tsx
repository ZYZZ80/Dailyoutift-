...
export default function DailyOutfitPage({
  ...
  const outfitItems = todayOutfit
    ? wardrobe.filter((item) => todayOutfit.itemIds.includes(item.id))
    : []
  ...
  async function runAgent() {
    if (wardrobe.length < 2) {
      setError('Add at least 2 clothing items first!')
      return
    }
    ...
    try {
      ...
      const savedOutfit: OutfitSuggestion = {
    ...
    if (primaryPhoto && config.provider !== 'ollama' && outfitResult) {
      ...
      try {
        const items = wardrobe.filter((item) =>
          outfitResult!.itemIds.includes(item.id),
        )
          ...
          <div className="flex items-center gap-2">
            {PHOTO_LABELS.map((label, i) => (
              <div key={label} className="flex flex-col items-center gap-1">
                <div
                  ...
                  onClick={() => {
                    if (profilePhotos[i]) setActivePhotoIdx(i)
                    else openPhotoUpload(i)
                  }}
                >
                  {profilePhotos[i] ? (
                    <>
            ...
            <p className="text-xs text-gray-400 ml-2 leading-tight">
              {profilePhotos.length > 0
                ? 'Tap a photo to select for try-on'
          ...
          <div className="flex items-center gap-3 text-xs">
            <div
              className={`flex items-center gap-1.5 ${
                agentStep === 'outfit' ? 'text-charcoal font-medium' : 'text-gray-300'
              }`}
            >
              <span
                className={`w-2 h-2 rounded-full ${
                  agentStep === 'outfit' ? 'bg-blush animate-pulse' : 'bg-gray-200'
                }`}
              />
              Picking outfit
            </div>
            ...
            <div
              className={`flex items-center gap-1.5 ${
                agentStep === 'preview' ? 'text-charcoal font-medium' : 'text-gray-300'
              }`}
            >
              <span
                className={`w-2 h-2 rounded-full ${
                  agentStep === 'preview' ? 'bg-blush animate-pulse' : 'bg-gray-200'
                }`}
              />
              Generating photo
            </div>
      ...
      {!todayOutfit && !isRunning && (
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-16 text-center">
          ...
          <p className="text-sm text-gray-400 max-w-xs mx-auto">
            Select an occasion and hit "Style Me" — your AI stylist will put together the perfect look.
          </p>
        </div>
      )}
          ...
          <div className="space-y-4">
            ...
            {showTips && (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3 text-sm text-gray-600 leading-relaxed">
                <p>💡 <strong>Accessorise:</strong> A belt, watch, or simple necklace can elevate this outfit.</p>
                <p>👟 <strong>Shoe swap:</strong> Try white sneakers, loafers, or boots depending on the vibe.</p>
                <p>🎨 <strong>Colour rule:</strong> Stick to 2–3 colours max.</p>
                <p>📦 <strong>Layer it:</strong> Add a jacket, blazer, or overshirt.</p>
                <p>🧺 <strong>Laundry tip:</strong> Items worn 2+ times are flagged in your wardrobe.</p>
              </div>
            )}
          </div>
