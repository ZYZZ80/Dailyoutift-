...
import { Shirt, Sparkles, History, Settings, Menu, X, LogOut, Loader2, CalendarDays, Wand2 } from 'lucide-react'
import { getConfig, getWardrobe, getOutfits, getProfilePhotos, saveConfig, saveWardrobe, saveOutfit as saveOutfitLocal, saveProfilePhotos, type AppConfig } from './lib/storage'
import { auth, FIREBASE_ENABLED } from './lib/firebase'
import { checkProxy } from './lib/claude'
import { addItemCloud, saveOutfitCloud, syncFromCloud, saveConfigCloud, uploadProfilePhoto } from './lib/cloud'
...
function mergeById<T extends { id: string }>(local: T[], cloud: T[]): T[] {
  return Array.from(new Map([...cloud, ...local].map((i) => [i.id, i])).values())
}
function mergeOutfits(local: OutfitSuggestion[], cloud: OutfitSuggestion[]): OutfitSuggestion[] {
  return Array.from(new Map([...cloud, ...local].map((i) => [i.date, i])).values()).sort((a, b) => b.date.localeCompare(a.date)).slice(0, 90)
}
...
export default function App() {
  ...
  async function syncUserData(userId: string) {
    const lw = getWardrobe(), lo = getOutfits(), lp = getProfilePhotos(), lc = getConfig()
    try {
      const snap = await withTimeout(syncFromCloud(userId), 8000)
      const mw = mergeById(lw, snap.wardrobe), mo = mergeOutfits(lo, snap.outfits)
      const mp = snap.profilePhoto ? Array.from(new Set([snap.profilePhoto, ...lp])).slice(0, 5) : lp
      saveWardrobe(mw); mo.forEach((o) => saveOutfitLocal(o)); if (mp.length > 0) saveProfilePhotos(mp)
      if (snap.config) { saveConfig(snap.config); setConfig(snap.config) }
      else if (isConfigured(lc)) saveConfigCloud(userId, lc).catch(() => {})
      await Promise.allSettled([...mw.map((i) => addItemCloud(userId, i)), ...mo.map((o) => saveOutfitCloud(userId, o)), ...lp.slice(0, 1).map((ph) => uploadProfilePhoto(userId, ph))])
    } catch { /* cloud unavailable */ } finally { refresh() }
  }
